...
namespace demedallo\BlockchainPHP;
use demedallo\BlockchainPHP\Block;
...
class Blockchain {
    ...
    public function getLastBlock($file_name) {
        if (!file_exists($file_name))
            return false;
        $size = filesize($file_name);
        $fp = fopen($file_name, 'rb');
        ...
        while (ftell($fp) < $size) {
            ...
            $blockchain = array(
                ...
                "datalen" => $datalen,
                "data" => wordwrap($data, 100));
            $blockchain = json_decode(json_encode($blockchain));
            
        }
        fclose($fp);
        return $blockchain;
        #return json_encode($blockchain, JSON_PRETTY_PRINT);
    }
